"""Return on assets using average total assets when available."""
...
class ReturnOnAssetsFactor:
    ...
    def calculate(
        ...
        *,
        ...
        filings = snapshot_accessions(current, "net_income", "total_assets") + snapshot_accessions(
            prior,
            "total_assets",
        )
        ...
        if ending_assets is not None and beginning_assets is not None:
            ...
            inputs["average_total_assets"] = avg_assets
            warnings.append("roa_fallback_ending_assets")
        else:
            return invalid_result(
                ticker=current.ticker,
                cik=current.cik,
